...
df = pd.DataFrame(columns=["species","chromosome","%div","type","frequence"])
        ...
        if line1[0].isnumeric():
          ...
          if chrom != line1[4] :
            if chrom != '':
              for key in LINE:
                l=len(LINE[key])
                newline=[assembly,chrom,key/10,"LINE",l]               
                df.loc[len(df)] = newline
              print(df)
              LINE=dict()
              for key in SINE:
                s=len(SINE[key])
                newline=[assembly,chrom,key/10,"SINE",s]               
                df.loc[len(df)] = newline
              print (df)
              SINE=dict()
...
df.to_csv(outputdir+"/LINESINE%div.tsv", sep = "\t")
