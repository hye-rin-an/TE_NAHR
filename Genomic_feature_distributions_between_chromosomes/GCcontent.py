...
directory='/home/knam/work/Metazoa_holocentrism/ncbi-genomes-2023-01-17'
outputdir='/home/knam/work/Metazoa_holocentrism/repeatmasker/result'

df=pd.DataFrame(columns=["assembly","chromosome","start_position","GC","non_GC","total"])

for root, dirs, files in os.walk(directory):
  for filename in files:
    if fnmatch.fnmatch(filename, '*.fna'):
...
#        print(line)
        ...
        if not line:
          ...
          for x in range (0,len(GC)):
            total_count.append (nonGC[x]+GC[x])
 
          newdf=pd.DataFrame({"assembly":species,"chromosome":chromo,"Startpoint":startpoint,"GCcount":GC,"non_GCcount":nonGC,"total":total_count})
